class TodosController < ApplicationController
  ...
  def index
    ...
    if session[:user_id].nil?
      ...
      Todo.connection.execute("UPDATE todos set todos.todo_urgence = 10/((todos.todo_deadline - now()) - todos.todo_timeremaining/8) WHERE todos.todo_complete = false or todos.todo_complete is null")
      ...
      @new_todo = Todo.new
      @todo_today = @todo_items.where("todos.todo_category<>'Personal' AND todo_urgence>0 AND (todo_urgence >= 11 OR DATE(todo_deadline) = curdate())").order("todo_complete ASC, (todo_urgence + todo_importance) DESC, todo_item ASC")
